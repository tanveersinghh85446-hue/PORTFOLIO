...
function ProjectCard({ title, description, image, tech, github, accentHex, index }) {
  return (
    <FadeIn delay={index * 0.07} className="h-full">
      <motion.article
        ...
        <div className="relative overflow-hidden shrink-0">
          <div className="absolute top-0 left-0 right-0 h-0.75 z-10" style={{ background: accentHex }} />
          <motion.img
            src={resolveImage(image)} alt={title}
        ...
        <div className="p-5 flex flex-col flex-1">
          <h3 className="font-['Syne',sans-serif] font-bold text-[1.05rem] text-white mb-2 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full shrink-0 inline-block" style={{ background: accentHex }} />
            {title}
          </h3>
          <p className="text-sm text-gray-500 leading-[1.65] mb-4 flex-1">{description}</p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-4">
            {tech.map((t, i) => (
              <span key={i}
                ...
                style={{ background: `${accentHex}18`, color: accentHex, border: `1px solid ${accentHex}55` }}>
                {t}
              </span>
            ))}
          </div>
          ...
          <a href={github} target="_blank" rel="noopener noreferrer"
            ...
            onMouseEnter={e => e.currentTarget.style.setProperty('--btn-bg', accentHex)}
          >
        ...
        <section className="relative pt-20 pb-6 px-6 text-center overflow-hidden">
          ...
          <FadeIn>
            <span className="font-['Syne',sans-serif] text-xs font-bold tracking-[0.14em] uppercase text-indigo-400 inline-flex items-center gap-2 mb-5">
              My Work
              <span className="inline-block h-px w-14 bg-white/[0.07]" />
            </span>
            <h1 className="font-['Syne',sans-serif] text-[clamp(2.2rem,6vw,4rem)] font-extrabold text-white leading-[1.08] mb-4">
              Projects &{" "}
              <span className="[-webkit-text-stroke:1px_rgba(79,110,247,0.7)] text-transparent">Case Studies</span>
            </h1>
