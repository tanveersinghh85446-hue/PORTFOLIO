  ...
  return (
    <>
      ...
      <nav className="flex items-center justify-between px-8 h-16 bg-white border-b border-gray-200 sticky top-0 z-100">
        ...
        <Link
          to="/"
          className="text-xl font-semibold tracking-tight text-gray-900 font-['Syne',sans-serif]"
        >
        ...
        >
          <span
            className={`block w-5 h-0.5 bg-gray-800 rounded transition-all duration-300 ${isOpen ? "rotate-45 translate-y-2" : ""}`}
          />
          <span
            className={`block w-5 h-0.5 bg-gray-800 rounded transition-all duration-300 ${isOpen ? "opacity-0 scale-x-0" : ""}`}
          />
          <span
            className={`block w-5 h-0.5 bg-gray-800 rounded transition-all duration-300 ${isOpen ? "-rotate-45 -translate-y-2" : ""}`}
          />
      ...
      {/* ── Mobile Dropdown ── */}
      <div
        className={`md:hidden flex-col gap-1 px-4 pb-4 pt-2 bg-white border-b border-gray-200 sticky top-16 z-99 ${isOpen ? "flex" : "hidden"}`}
      >
        <NavLink
          to="/"
